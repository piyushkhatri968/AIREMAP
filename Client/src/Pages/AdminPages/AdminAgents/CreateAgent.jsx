...
import useCreateAgent from "../../../hooks/Adminhooks/useCreateAgent";

const CreateAgent = ({ setAddAgentPopup }) => {
  ...
  const { createAgentMutation, isPending } = useCreateAgent({
    setAddAgentPopup,
  });

  const handleSubmit = (e) => {
    e.preventDefault();
    createAgentMutation(formData);
  };
  ...
  return (
    <div className="w-full min-h-screen bg-black/40 flex items-center justify-center backdrop-blur-sm fixed inset-0 z-[9999]">
      ...
      >
        {/* Close Button */}
        <button
          onClick={() => setAddAgentPopup(false)}
